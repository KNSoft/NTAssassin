#include "..\NTAssassin\3rdParty\include\Wintexports\Wintexports.h"
#include "..\NTAssassin\include\NTAssassin\NTAssassin_Type.h"
...
NTSTATUS WINAPI Hijack_LoadProcAddr_InjectThread(LPVOID lParam)
{
    ...
    // Start to load
    HMODULE hDLL = NULL, hNtDLL = CURRENT_NTDLL_BASE;
    // Get DLL handle if already loaded
    PLDR_DATA_TABLE_ENTRY pLdrNodeHead, pLdrNode;
    pLdrNodeHead = CONTAINING_RECORD(NtCurrentPeb()->Ldr->InLoadOrderModuleList.Flink, LDR_DATA_TABLE_ENTRY, InLoadOrderModuleList);
