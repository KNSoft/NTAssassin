...
#define Con_GetStdHandle(i) (\
    i == STD_INPUT_HANDLE ? (NtCurrentPeb()->ProcessParameters->StandardInput) : (\
        i == STD_OUTPUT_HANDLE ? (NtCurrentPeb()->ProcessParameters->StandardOutput) : (\
            i == STD_ERROR_HANDLE ? (NtCurrentPeb()->ProcessParameters->StandardError) : NULL\
        )\
    )\
)
...
NTA_API DWORD NTAPI Con_Write(HANDLE ConHandle, _In_reads_bytes_(Length) PVOID Buffer, _In_ ULONG Length);
