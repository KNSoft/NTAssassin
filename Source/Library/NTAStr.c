...
_Success_(return >= 0)
INT CDECL Str_VPrintfExW(_Out_writes_z_(DestCchSize) PWSTR Dest, _In_ SIZE_T DestCchSize, _In_z_ _Printf_format_string_ PCWSTR Format, _In_ va_list ArgList)
{
    return vswprintf_s(Dest, DestCchSize, Format, ArgList);
}

_Success_(return >= 0)
INT CDECL Str_VPrintfExA(_Out_writes_z_(DestCchSize) PSTR Dest, _In_ SIZE_T DestCchSize, _In_z_ _Printf_format_string_ PCSTR Format, _In_ va_list ArgList)
{
